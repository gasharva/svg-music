...
{
    private sealed record BeamStrip(SvgDirectPath Path, double Left, double Right, double Top, double Bottom, double Thickness);
    ...
    public void Resolve(AnalysisResult analysis, RecognitionConfig config)
    {
        ...
        foreach (var staff in analysis.Staves)
        {
            var members = analysis.Events
                ...
                .Where(x => x.StemX!.Value >= beam.Left - staff.Space * .35 && x.StemX.Value <= beam.Right + staff.Space * .35)
                .Select(note => new { Note = note, Stem = FindStem(analysis, note, staff), Slice = SliceAtX(beam.Path, Math.Clamp(note.StemX!.Value, beam.Left, beam.Right)) })
                .Where(x => x.Stem is not null && x.Slice.HasValue)
                .Where(x => IntervalDistance(x.Stem!.Top, x.Stem.Bottom, x.Slice!.Value.Top, x.Slice.Value.Bottom) <= Math.Max(staff.Space * .65, beam.Thickness * 1.5))
            ...
            for (var i = 0; i < groups.Count; i++)
            {
                ...
                foreach (var member in groups[i])
                {
                    ...
                    if (note.BeamCount == 1) note.Type = "eighth";
    ...
    private static IReadOnlyList<BeamStrip> FindBeamStrips(AnalysisResult analysis)
    {
        ...
        foreach (var path in analysis.DirectPaths)
        {
            ...
            var left = points.Min(x => x.X); var right = points.Max(x => x.X);
            var top = points.Min(x => x.Y); var bottom = points.Max(x => x.Y);
            ...
            if (width < staff.Space * 1.15 || height > staff.Space * 6.0) continue;
    ...
    private static (double Top, double Bottom)? SliceAtX(SvgDirectPath path, double x)
    {
        ...
        for (var i = 0; i < contour.Count; i++)
        {
            ...
            if (Math.Abs(dx) < .001)
            {
                if (Math.Abs(x - a.X) <= .02) { ys.Add(a.Y); ys.Add(b.Y); }
                continue;
            }
            var t = (x - a.X) / dx;
