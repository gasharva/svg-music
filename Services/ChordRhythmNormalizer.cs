...
public sealed class ChordRhythmNormalizer
{
    public void Normalize(AnalysisResult analysis)
    {
        foreach (var staff in analysis.Staves)
        {
            ...
            // Opposite voices can legitimately use almost the same stem X. They are not one chord:
            // a dotted down-stem chord must not donate its half-note rhythm to an overlapping
            // up-stem quarter note. Normalize only within one stem direction.
