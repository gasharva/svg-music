    ...
    public void Resolve(AnalysisResult analysis)
    {
        ...
        foreach (var staffGroup in notes.GroupBy(x => x.StaffIndex))
        {
            ...
            foreach (var note in staffGroup)
            {
                ...
                // A normal chord has one physical stem. Do not reinterpret every notehead from that
                // same long line: low and high heads on one chord can lie on opposite sides of its
                // midpoint and would falsely acquire opposite directions. Only intervene when the
                // SVG actually contains two almost coincident stem candidates — genuine polyphony.
