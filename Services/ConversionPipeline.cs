...
public sealed class ConversionPipeline
{
    ...
    {
        ValidateInput(svgPath, catalogPath); config ??= new RecognitionConfig();
        var performance = new ConversionPerformance(); var total = Stopwatch.StartNew(); var watch = Stopwatch.StartNew();
        var parser = new SvgParser(); var document = parser.Load(svgPath); performance.ParseSvgMs = watch.Elapsed.TotalMilliseconds;
        watch.Restart(); var staves = parser.DetectStaves(document, config.StaffTolerance); performance.DetectStavesMs = watch.Elapsed.TotalMilliseconds;
        watch.Restart(); var uses = parser.ReadUses(document); var pageGeometry = parser.ReadPageGeometry(document); var lineSegments = parser.ReadLineSegments(document);
        lineSegments.AddRange(new CompoundVerticalStrokeExtractor().Extract(pageGeometry, staves, lineSegments)); performance.ReadInstancesMs = watch.Elapsed.TotalMilliseconds;
        var classifier = new SymbolClassifier(); var classification = classifier.Classify(svgPath, staves, catalogPath, config.RecognitionParallelism);
        new SourceFontSemanticNormalizer().Normalize(svgPath, staves, classification, lineSegments);
        var cp = classifier.LastPerformance; performance.LoadCatalogMs = cp.LoadCatalogMs; performance.ClassifyMs = cp.ClassifyMs;
        performance.GlyphInstances = cp.GlyphInstances; performance.UniqueGeometries = cp.UniqueGeometries; performance.CatalogGlyphs = cp.CatalogGlyphs;
        performance.MaskComparisons = cp.MaskComparisons; performance.VectorComparisons = cp.VectorComparisons; performance.CatalogCacheHit = cp.CatalogCacheHit;
        watch.Restart(); var analysis = new MusicSemanticRecognizer().Recognize(uses, staves, classification, config);
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(musicXmlPath); config ??= new RecognitionConfig(); var result = Analyze(svgPath, catalogPath, config);
        Directory.CreateDirectory(Path.GetDirectoryName(Path.GetFullPath(musicXmlPath))!); var watch = Stopwatch.StartNew();
        new MusicXmlWriter().Write(musicXmlPath, result.Analysis, config); new MusicXmlStemPostProcessor().Apply(musicXmlPath, result.Analysis);
        new MusicXmlGraceNotePostProcessor().Apply(musicXmlPath, result.Analysis); new MusicXmlSvgLayoutPostProcessor().Apply(musicXmlPath, result.Analysis);
        new MusicXmlRestYPostProcessor().Apply(musicXmlPath, result.Analysis); new MusicXmlTieSidePostProcessor().Apply(musicXmlPath, result.Analysis);
        new MusicXmlNaturalAccidentalPostProcessor().Apply(musicXmlPath, result.Analysis); new MusicXmlOrnamentGeometryPostProcessor().Apply(musicXmlPath, result.Analysis);
        new MusicXmlShortTrillPostProcessor().Apply(musicXmlPath); new MusicXmlVoiceLayoutPostProcessor().Apply(musicXmlPath, result.Analysis);
        new MusicXmlRestVoiceConflictPostProcessor().Apply(musicXmlPath); new MusicXmlGeneralMultiVoicePostProcessor().Apply(musicXmlPath);
        ...
        string? analysisPath = null; string? classificationPath = null;
        if (writeDiagnostics) { analysisPath = Path.ChangeExtension(musicXmlPath, ".analysis.json"); classificationPath = Path.ChangeExtension(musicXmlPath, ".classification.json"); WriteJson(analysisPath, result.Analysis); WriteJson(classificationPath, result.Classification); WriteJson(Path.ChangeExtension(musicXmlPath, ".performance.json"), result.Performance); }
    ...
    private static void ValidateInput(string svgPath, string catalogPath) { ArgumentException.ThrowIfNullOrWhiteSpace(svgPath); ArgumentException.ThrowIfNullOrWhiteSpace(catalogPath); if (!File.Exists(svgPath)) throw new FileNotFoundException("SVG not found.", svgPath); if (!File.Exists(catalogPath)) throw new FileNotFoundException("Reference catalog not found.", catalogPath); }
