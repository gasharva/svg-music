...
Small isolated experiment: feed individual SVG glyph candidates from the existing step-by-step diagnostics into PP-OCRv5 Latin and inspect what it predicts.

The PoC intentionally does **not** change the production SVG-to-MusicXML pipeline.

## Input

Use a directory containing standalone glyph SVG files. The current diagnostics already emit these through `MusicSymbolSvgExporter` into a `music-symbols` directory.

## Run

```powershell
dotnet run --project Experiments/GlyphOcrPoc -- `
  <path-to-step-by-step-output>\music-symbols `
  artifacts\glyph-ocr
```
...
This first experiment deliberately keeps the OCR detector/classifier/recognizer pipeline intact. If single-glyph detection turns out to be the weak link, the next experiment should call the recognition model directly and bypass text detection entirely.
