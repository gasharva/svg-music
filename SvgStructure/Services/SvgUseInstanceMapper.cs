...
///
/// Svg.Skia's SourceElementAddress is the element address generated by SvgElementAddressKeyCache:
/// the SvgDocument itself has no address and its direct children are "0", "1", ... .
/// Keep exactly that address convention here so retained draw commands can be matched to the
/// referenced definition subtree, then choose the nearest concrete <use> instance for that glyph.
/// </summary>
public sealed class SvgUseInstanceMapper
{
    public IReadOnlyList<RawPrimitive> Map(SKSvg svg, IReadOnlyList<RawPrimitive> primitives)
    {
        ...
        Walk(sourceDocument, null, nodes);

        var idToPath = nodes
            .Where(x => !string.IsNullOrWhiteSpace(x.Id) && !string.IsNullOrWhiteSpace(x.Path))
            .GroupBy(x => x.Id!, StringComparer.Ordinal)
            .ToDictionary(x => x.Key, x => x.First().Path!, StringComparer.Ordinal);

        var uses = nodes
            .Where(x => x.IsUse && !string.IsNullOrWhiteSpace(x.Path) && !string.IsNullOrWhiteSpace(x.Reference))
    ...
    private static void Walk(object node, string? path, ICollection<SourceNode> result)
    {
        ...
        for (var i = 0; i < children.Length; i++)
        {
            ...
            Walk(children[i], childPath, result);
        }
    }
