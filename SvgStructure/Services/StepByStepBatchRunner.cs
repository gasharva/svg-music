...
public sealed class StepByStepBatchRunner
{
    ...
    private StepByStepItemResult Process(string svgPath,string artifactsFolder,MeterResolver meterResolver,ClefResolver clefResolver,AccidentalResolver accidentalResolver,DiagnosticNumberRecognizer diagnosticNumberRecognizer,DiagnosticClefRecognizer diagnosticClefRecognizer)
    {
        ...
        try {
            ...
            _partMeasureOverlayRenderer.Render(structure,Path.Combine(itemDirectory,"measures.png")); _primitiveOverlayRenderer.Render(primitives,Path.Combine(itemDirectory,"classified.png")); _meterOverlayRenderer.Render(structure,meters,clefs,ledgerLines,noteHeads,accidentals,stems,logicalGrid,Path.Combine(itemDirectory,"meters.png"));
            WriteResolutionJson(Path.Combine(itemDirectory,"structure.json"),fileName,structure,primitives,musicSymbols,meters,logicalGrid,clefs,ledgerLines,noteHeads,accidentals,stems);
