...
public sealed class StepByStepReportBuilder
{
    public void WriteHtml(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        html.AppendLine("<p>Step 1: PartMeasureResolver. Step 2: PrimitiveResolver. Step 3: MeterResolver. Overlays are diagnostics only.</p>");
        html.AppendLine("<table><thead><tr><th>Source</th><th>P+M blocks</th><th>Primitives</th><th>Meters</th><th>State</th></tr></thead><tbody>");
    ...
    public void WriteMarkdown(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        md.AppendLine();
        md.AppendLine("Latest results produced by the CI pipeline: PartMeasureResolver → PrimitiveResolver → MeterResolver.");
        md.AppendLine();
        md.AppendLine("| SVG | P+M blocks | Primitives | Meters | State |");
        md.AppendLine("|---|---|---|---|---|");
