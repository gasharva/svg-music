...
public sealed class StepByStepReportBuilder
{
    public void WriteHtml(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        html.AppendLine("<p>Every SVG is processed through the current geometry → systems → parts/measures → primitive classification pipeline.</p>");
        html.AppendLine("<table><thead><tr><th>Source</th><th>Measure regions</th><th>Primitive classification</th><th>Detected structure</th></tr></thead><tbody>");
    ...
    public void WriteMarkdown(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        md.AppendLine();
        md.AppendLine("Latest results produced by the CI step-by-step pipeline.");
        md.AppendLine();
        md.AppendLine("| SVG | Measures | Classified | Structure |");
        md.AppendLine("|---|---|---|---|");
        ...
        foreach (var item in items)
        {
            ...
            md.AppendLine(
                ...
                $"[![measures]({dir}/measures.png)]({dir}/measures.png) | " +
                $"[![classified]({dir}/classified.png)]({dir}/classified.png) | " +
                $"lines={item.LineCount}<br>systems={item.SystemCount}<br>parts={item.PartCount}<br>measures={item.MeasureCount}<br>[json]({dir}/structure.json) |");
        }
