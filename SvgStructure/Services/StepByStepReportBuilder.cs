...
public sealed class StepByStepReportBuilder
{
    ...
    public void WriteHtml(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        html.AppendLine("<p>PartMeasureResolver → PrimitiveResolver → MusicSymbolResolver → MeterResolver → logical grid → ClefResolver → LedgerLineResolver → NoteHeadResolver → AccidentalResolver → StemDetector → BeamResolver → NoteFlagResolver → ArcResolver → RestResolver. Overlays are diagnostics only.</p>");
        ...
        foreach (var item in items)
        {
            ...
            html.Append($"<td><a href=\"{dir}/meters.png\"><img class=\"preview\" src=\"{dir}/meters.png\"></a><br><b>meters: {item.MeterCount}</b><br><b>clefs: {item.ClefCount}</b><br><b>ledger ladders: {item.LedgerLineCount}</b><br><b>note heads: {item.NoteHeadCount}</b><br><b>accidentals: {item.AccidentalCount}</b><br><b>rests: {item.RestCount}</b><br><br><b>inputs:</b><br><a href=\"{dir}/meter-inputs/index.html\">meter</a> · <a href=\"{dir}/clef-inputs/index.html\">clef</a> · <a href=\"{dir}/notehead-inputs/index.html\">note-head</a> · <a href=\"{dir}/flag-inputs/index.html\">flag</a> · <a href=\"{dir}/arc-inputs/index.html\">arc</a> · <a href=\"{dir}/rest-inputs/index.html\"><b>rest</b></a></td>");
    ...
    public void WriteMarkdown(string outputPath, IReadOnlyList<StepByStepItemResult> items)
    {
        ...
        md.AppendLine();
        md.AppendLine("Latest results: PartMeasureResolver → PrimitiveResolver → MusicSymbolResolver → MeterResolver → logical grid → ClefResolver → LedgerLineResolver → NoteHeadResolver → AccidentalResolver → StemDetector → BeamResolver → NoteFlagResolver → ArcResolver → RestResolver.");
        md.AppendLine();
        ...
        foreach (var item in items)
        {
            ...
            md.AppendLine(
                ...
                $"[![symbols]({dir}/meters.png)]({pageDir}/meters.png)<br>meters={item.MeterCount}<br>clefs={item.ClefCount}<br>ledgerLadders={item.LedgerLineCount}<br>noteHeads={item.NoteHeadCount}<br>accidentals={item.AccidentalCount}<br>rests={item.RestCount}<br>[meter inputs]({pageDir}/meter-inputs/) · [clef inputs]({pageDir}/clef-inputs/) · [note-head inputs]({pageDir}/notehead-inputs/) · [flag inputs]({pageDir}/flag-inputs/) · [arc inputs]({pageDir}/arc-inputs/) · [rest inputs]({pageDir}/rest-inputs/) | " +
